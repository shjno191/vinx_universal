...
if (!code.includes('fn open_excel_at_sheet')) {
    const entryPoint = '#[cfg_attr(mobile, tauri::mobile_entry_point)]';
    if (code.includes(entryPoint)) {
        code = code.replace(entryPoint, commandCode + '\\n' + entryPoint);
    }
}

if (!code.includes('open_excel_at_sheet,')) {
    const generateHandler = 'tauri::generate_handler![';
    if (code.includes(generateHandler)) {
        code = code.replace(generateHandler, generateHandler + '\\n            open_excel_at_sheet,');
    }
}

fs.writeFileSync(path, code, 'utf8');
console.log('lib.rs correctly patched for open_excel_at_sheet');
